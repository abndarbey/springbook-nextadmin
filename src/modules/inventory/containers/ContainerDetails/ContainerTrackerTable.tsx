...
import {
    useContainerTrackerLogsQuery,
    ContainerTrackerLog,
    ContainerTrackerLogsResult,
    SortByOption,
    SortDir,
    FilterOption
...
interface ContainerTrackerLogTableProps {
    data: ContainerTrackerLogsResult
    batchViewAction?: any
    filterAction?: any
...
export default function ContainerTrackerTable(props: ContainerTrackerTableProps) {
    ...
    // Batch Actions
    const batchViewAction = (selectedRecords: ContainerTrackerLog[]) => {
    ...
    return (
        <Fragment>
            <ContainerTrackerLogTable
                data={data?.containerTrackerLogs!}
...
const ContainerTrackerLogTable = (props: ContainerTrackerLogTableProps) => {
    const [pageSize, setPageSize] = useState(PAGE_SIZES[1])
    const [page, setPage] = useState(1)
    const [records, setRecords] = useState<ContainerTrackerLog[]>(props.data.containerTrackerLogs.slice(0, pageSize))
    const [selectedRecords, setSelectedRecords] = useState<ContainerTrackerLog[]>([])

    useEffect(() => {
        const from = (page - 1) * pageSize
        const to = from + pageSize
        setRecords(props.data.containerTrackerLogs.slice(from, to))
    }, [page, pageSize, props.data.containerTrackerLogs])
