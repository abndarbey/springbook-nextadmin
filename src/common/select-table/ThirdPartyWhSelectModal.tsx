...
import {
    FilterOption,
    ThirdPartyWarehouse,
    ThirdPartyWarehousesResult,
    SortByOption,
    SortDir,
    useThirdPartyWarehousesQuery,
...
export default function ThirdPartyWarehouseSelectModal(props: ISelectModalProps) {
    ...
    return (
        <Modal
            ...
            overlayBlur={3}
        >
            <PageHeader title='Select ThirdPartyWarehouse' />
            <ThirdPartyWarehouseTable
                data={data?.thirdPartyWarehouses!}
...
interface ThirdPartyWarehouseTableProps {
    data: ThirdPartyWarehousesResult
    batchViewAction?: any
    filterAction?: any
...
const ThirdPartyWarehouseTable = (props: ThirdPartyWarehouseTableProps) => {
    const theme = useMantineTheme()

    const [pageSize, setPageSize] = useState(PAGE_SIZES[1])
    const [page, setPage] = useState(1)
    const [records, setRecords] = useState<ThirdPartyWarehouse[]>(props.data.thirdPartyWarehouses.slice(0, pageSize))
    const [selectedRecords, setSelectedRecords] = useState<ThirdPartyWarehouse[]>([])
    ...
    useEffect(() => {
        ...
        setRecords(props.data.thirdPartyWarehouses.slice(from, to))
    }, [page, pageSize, props.data.thirdPartyWarehouses])
